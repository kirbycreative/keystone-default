...
@php($instanceNumber = page()->counter('divider'))

<style>
    @once
        .custom-divider { position: absolute; overflow: hidden; line-height: 0; }
        .custom-divider svg { position: relative; height: 100%; transform: translateX(-50%); }
    @endonce

    #divider-{{ $instanceNumber }} {
        width: {{ $width }};
        height: {{ $height }};
        left: {{ $left }};
        top: {{ $top }};
        @if ($mirrorX) transform: scaleX(-1); @endif
        @if ($mirrorY) transform: scaleY(-1); @endif
    }
    #divider-{{ $instanceNumber }} .shape-fill { fill: {{ $color }}; }
    #divider-{{ $instanceNumber }} svg { width: {{ $shapeWidth }}; left: {{ $centerX }}; }
</style>

@if ($type === 'waves-opacity')
    <div id="divider-{{ $instanceNumber }}" {{ $attributes->class(['custom-divider', 'wave-divider']) }}>
...
@elseif ($type === 'tilt')
    <div id="divider-{{ $instanceNumber }}" {{ $attributes->class(['custom-divider', 'tilt-divider']) }}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
            <path d="{{ $direction === 'ltr' ? 'M1200 120L0 16.48 0 0 1200 0 1200 120z' : 'M0 120L1200 16.48 1200 0 0 0 0 120z' }}" class="shape-fill" />
        </svg>
    </div>
@elseif ($type === 'waves')
    <div id="divider-{{ $instanceNumber }}" {{ $attributes->class(['custom-divider', 'wave-divider']) }}>
